--DB CREATION SCRIPTS OFTEN BEGIN WITHDROP STATEMENT
--AVOID CLASES WITH EXISTING TABLES
...
SELECT MAX(WEIGHT) From BEAR WHERE WEIGHT < (SELECT Max(WEIGHT) FROM BEAR);

SELECT CAVE.CAVE_NAME AS CAVE_NAME,CAVE.CAVE_ID AS CAVE_ID, 
COUNT(BEAR.CAVE_ID) AS CAVE_COUNT FROM BEAR 
INNER JOIN CAVE ON BEAR.CAVE_ID = CAVE.CAVE_ID GROUP BY  CAVE.CAVE_NAME,CAVE.CAVE_ID,BEAR.CAVE_ID ;


Alter table Beehive add constraint CK_Beehive
